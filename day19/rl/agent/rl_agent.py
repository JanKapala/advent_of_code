...
from day19.rl.agent.replay_buffer import ReplayBuffer
...
class TD3Agent:
    def __init__(
        ...
        self.action_spac = action_space
        self.observation_space = observation_space
        self.state_encoder = state_encoder or StateEncoder()
        self.device = device

        # Actor
        self.μ_θ = Actor(layer_sizes=actor_layer_sizes).to(device)
        ...
        # Critics
        self.Q1_Φ = Critic(layer_sizes=critic_layer_sizes).to(device)
        self.Q2_Φ = Critic(layer_sizes=critic_layer_sizes).to(device)
    ...
    def act(self, S):
        ...
        with torch.no_grad():
            ...
            A = self._prepare_action(weights)
    ...
    def _actor_train_step(self, batch):
        if self.update_times_counter % self.policy_delay == 0:  # TODO: Move to common part
            S, _, _, _, _ = batch

            for p in self._Q_Φ_params: # tODO: check if this "require_grad" staff off/on is really needed
                p.requires_grad = False
    ...
    def _prepare_observation(self, observation: Observation):
        return Tensor(flatten(self.observation_space, observation)).to(self.device),

    def _prepare_action(self, weights):
        weights = weights.cpu().squeeze() # TODO: device handling, weights to action
        return action
    ...
    def _update_time(self):
        return self.steps_counter >= self.train_after and self.steps_counter % self.learning_freq == 0
